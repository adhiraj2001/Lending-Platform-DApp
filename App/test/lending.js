...
contract('Lending', async ([owner, alice, bob]) => {
    ...
    it('should create a new proposal', async () => {
        try {
            const { logs } = await contractInstance.createProposal(
                100,
                50,
    ...
    it('should accept the lender', async () => {
        try {
            ...
            await contractInstance.acceptLender(0, 0, {
                from: alice,
                gasPrice: 8000000000,
                gas: 4700000,
            });
    ...
    it('should repay the amount to lender', async () => {
        try {
            ...
            await contractInstance.sendETHtoContract({
                from: bob,
                value: 100000,
            });

            const borrow = await contractInstance.acceptLender(0, 0, {
                from: alice,
            });
            ...
            const repay = await contractInstance.repayLoan(0, {
                from: alice,
                value: 200000,
            });
